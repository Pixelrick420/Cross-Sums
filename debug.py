    ...
    def display(self):
        ...
        def displayRow(array, row, isSum = False):
            for i in range(len(array)):
                print(array[i], end=(' ' * (4 - len(str(array[i])))))
            if not isSum:
    ...
    def solve(self):
        ...
        for row in order:
            if self.rowSums[row] > 0:
                ...
                for solution in possible:
                    if any((row, col) in self.solution for col in solution):
                        confirmable.append(solution)
                ...
                for col in discard:
                    if (row, col) not in self.solution:
        ...
        for col in order:
            if self.colSums[col] > 0:
                ...
                confirmable = []
                for solution in possible:
                    if any((row, col) in self.solution for row in solution):
                        confirmable.append(solution)
                    rows.update([row for row in solution])
                
                if len(confirmable) == 1:
                    rows = [(row, col) for row in confirmable[0]]
